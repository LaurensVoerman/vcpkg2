...
//    avpicture_fill((AVPicture *) (m_frame_rgba).get(), &(m_buffer_rgba[0])[0], AV_PIX_FMT_RGB24, width(), height());
	AVPicture *picture = (AVPicture *)(m_frame_rgba).get();
	av_image_fill_arrays(picture->data, picture->linesize, &(m_buffer_rgba[0])[0], AV_PIX_FMT_RGB24, width(), height(), 1);
...
void FFmpegDecoderVideo::decodeLoop()
{
    ...
    double pts;
    {
        ...
        if (num_cpus > RESERVERD_CORES + 1) {
            int cpu = next_cpu;
            ++next_cpu;
            if (next_cpu >= num_cpus) next_cpu -= num_cpus - RESERVERD_CORES;
            if (cpu >= num_cpus) cpu -= num_cpus - RESERVERD_CORES;
            OpenThreads::SetProcessorAffinityOfCurrentThread(cpu);
            OSG_WARN << "FFmpegDecoderVideo::run : OpenThreads::SetProcessorAffinityOfCurrentThread" << cpu << std::endl;
        }
    }
    while (! m_exit)
    {
...
//    avpicture_fill((AVPicture *) (m_frame_rgba).get(), &(m_buffer_rgba[m_writeBuffer])[0], AV_PIX_FMT_RGB24, width(), height());
	AVPicture *picture = (AVPicture *)(m_frame_rgba).get();
	av_image_fill_arrays(picture->data, picture->linesize, &(m_buffer_rgba[m_writeBuffer])[0], AV_PIX_FMT_RGB24, width(), height(), 1);
